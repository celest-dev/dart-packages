...
base class NativeAuthDesktop extends NativeAuthPlatform {
  NativeAuthDesktop({Logger? logger})
  ...
  NativeAuthCallbackSession startAuthorizationRedirect({
    ...
    final (port, expectedPath) = switch (callbackScheme) {
      CallbackSchemeLocalhost(:final port, :final path) => (port, path),
      CallbackSchemeCustom() => throw ArgumentError.value(
          callbackScheme,
          'callbackScheme',
          'Custom schemes are not supported on this platform',
        ),
      CallbackSchemeHttps() => throw ArgumentError.value(
          callbackScheme,
          'callbackScheme',
    ...
    callback.complete(
      server.then((server) async {
        ...
        logger?.fine('Listening for callback on $callbackScheme');
